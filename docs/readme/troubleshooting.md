...
| `/quota_switch` fails    | Run `/quota_switch <id>` with an exact configured id and ensure that account has a non-empty `apiKey`.                                                                                          |
| Switched key not used    | Update the plugin and start a new request after the command completes; the next OpenCode Go request is overridden immediately. Also confirm `opencode auth list` includes `opencode-go`.       |
| Quota looks stale        | Run `/quota_refresh` to fetch fresh provider data and replace the local memory and disk cache immediately.                                                                                     |
