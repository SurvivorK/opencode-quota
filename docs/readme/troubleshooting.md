...
| Switched key not used    | Start a new request after the command completes. Confirm `opencode auth list` includes `opencode-go`; environment credentials can take precedence over stored auth.                            |
